...
def plotXY(X, Y, legend=True):
    """
        ...
        Input:
            Two options:
            1) X is a numpy array with two columns (the data matrix) and Y is the vector of labels (works for many classes).
            2) X and Y are two numpy arrays with two columns; each array is the data matrix for a class (works only for
            two classes).
    """
    if Y.ndim > 1:
        XX = np.concatenate((X, Y), axis=0)
        YY = np.concatenate((np.ones(X.shape[0], dtype=int), 2*np.ones(Y.shape[0], dtype=int)))
    else:
        XX = X
        YY = Y
    mod = np.unique(YY)
    if mod.size==2 and mod[0]<0:
        mod = mod[::-1]
    for icl, cl in enumerate(mod):
        plt.scatter(XX[YY==cl, 0], XX[YY==cl, 1], label=f'Class {cl}')
    plt.axis('equal')
    if legend:
        plt.legend()
